...
class BenchmarkAnalyzer:
    ...
    def analyze_parsing_trends(self):
        ...
        if len(cv_times) >= 2:
            ...
            print(f"  CV Parsing: {cv_trend:+.2f}ms "
                  f"({(cv_trend/cv_times[0]*100):+.1f}%)")
            print(f"  Job Parsing: {job_trend:+.2f}ms "
                  f"({(job_trend/job_times[0]*100):+.1f}%)")
    ...
    def analyze_matching_trends(self):
        ...
        if len(match_times) >= 2:
            ...
            print("\nTrend (latest vs. earliest):")
            print(f"  Match Time: {time_trend:+.2f}ms "
                  f"({(time_trend/match_times[0]*100):+.1f}%)")
            print(f"  Throughput: {throughput_trend:+.2f} matches/sec "
    ...
    def compare_latest_runs(self):
        ...
        if len(self.parsing_results) >= 2:
            ...
            print(f"  {'CV Parsing':<20} {cv_prev:<15.2f} {cv_latest:<15.2f} "
                  f"{cv_change:>+9.1f}%")
            ...
            print(f"  {'Job Parsing':<20} {job_prev:<15.2f} {job_latest:<15.2f} "
                  f"{job_change:>+9.1f}%")
        ...
        if len(self.matching_results) >= 2:
            ...
            print(f"  {'Single Match':<20} {match_prev:<15.2f} {match_latest:<15.2f} "
                  f"{match_change:>+9.1f}%")
            ...
            print(f"  {'Max Throughput':<20} {tp_prev:<15.2f} {tp_latest:<15.2f} "
                  f"{tp_change:>+9.1f}%")
    ...
    def generate_summary_report(self):
        ...
        print(f"Total Benchmark Files: "
              f"{len(self.parsing_results) + len(self.matching_results)}")
        ...
        if self.matching_results:
            ...
            for result in self.matching_results:
                all_match_times.append(
                    result['data']['summary']['single_match']['avg_time_ms']
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze benchmark results over time"
    )
    parser.add_argument(
        '--dir',
        type=str,
