...
def test_gap_analysis_required_skills(sample_cv, sample_job):
    ...
    assert "Kubernetes" in gaps.get("missing_required_skills") or "Kubernetes" in gaps.get("missing_preferred_skills")
...
def test_gap_analysis_strengths(sample_cv, sample_job):
    ...
    for skill in sample_cv["skills"]:
        if skill in sample_job["required_skills"] or skill in sample_job.get("preferred_skills", []):
