...
class TextFormatter:
    ...
    @staticmethod
    def format_key_value(data: Dict[str, Any], indent: int = 0, 
                        separator: str = ": ") -> str:
        """
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                lines.append(f"{indent_str}{key}:")
                lines.append(TextFormatter.format_list(
                    [str(v) for v in value], indent=indent + 1
                ))
            else:
    ...
    def format_box(text: str, width: int = 60, padding: int = 1) -> str:
        ...
        """
        lines = text.split('\n')
        padded_lines = [f"│{' ' * padding}{line.ljust(width - 2 * padding - 2)}{' ' * padding}│" 
                       for line in lines]
...
class MatchFormatter:
    ...
    def format_match_score(match_data: Dict[str, Any]) -> str:
        ...
        if score >= 80:
            interpretation = "EXCELLENT MATCH - Highly qualified"
        elif score >= 60:
            interpretation = "GOOD MATCH - Well qualified with some gaps"
        elif score >= 40:
            interpretation = "MODERATE MATCH - Significant development needed"
        else:
            interpretation = "WEAK MATCH - Major skill gaps"
    ...
    @staticmethod
    def format_match_report(job_title: str, company: str, 
    ...
    def format_learning_plan(plan: Dict[str, Any]) -> str:
        ...
        for priority in ['high', 'medium', 'low']:
            if by_priority[priority]:
                output += TextFormatter.format_header(
                    f"{priority.upper()} PRIORITY ITEMS", level=2
                )
