...
class Validator:
    ...
    @staticmethod
    def validate_range(
        value: float, min_val: float, max_val: float, field_name: str = "Field"
    ) -> None:
        """
...
class FileValidator(Validator):
    ...
    _SUPPORTED_EXTENSIONS = {".pdf", ".docx", ".doc", ".txt", ".json"}
    ...
    def validate_file_size(file_path: str, max_size_mb: Optional[float] = None) -> None:
        ...
        if max_size_mb is None:
            max_size_mb = FileValidator._MAX_FILE_SIZE_MB

        path = Path(file_path)
        size_mb = path.stat().st_size / (1024 * 1024)

        if size_mb > max_size_mb:
            raise ValidationError(f"File too large: {size_mb:.2f}MB (max: {max_size_mb}MB)")
...
class SkillValidator(Validator):
    ...
    _VALID_SKILL_LEVELS = ["beginner", "intermediate", "advanced", "expert"]
    ...
    def validate_skill_level(level: str) -> None:
        ...
        """
        Validator.validate_choices(level.lower(), SkillValidator._VALID_SKILL_LEVELS, "Skill level")
...
class JobValidator(Validator):
    ...
    def validate_job_description(description: str) -> None:
        ...
        if word_count < 50:
            raise ValidationError(f"Job description too short: {word_count} words (minimum: 50)")
...
class ConfigValidator(Validator):
    ...
    def validate_phone(phone: str) -> None:
        ...
        if len(cleaned) < 10 or len(cleaned) > 15:
            raise ValidationError(f"Phone number must be 10-15 digits, got {len(cleaned)}")
...
class LearningPlanValidator(Validator):
    ...
    def validate_learning_item(item: Dict[str, Any]) -> None:
        ...
        valid_types = ["course", "book", "tutorial", "practice", "project"]
