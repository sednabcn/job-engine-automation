...
def fix_file_indentation(input_path, output_path=None):
    ...
    while i < len(lines):
        ...
        method_match = re.match(r'^(\s+)def (end_sprint|check_quality_gates|stage_positioning|display_progress_dashboard)\(', line)
        ...
        if method_match:
            ...
            if current_indent > 4:
                ...
                while i < len(lines):
                    ...
                    if re.match(r'^    def \w+', next_line) and not re.match(r'^        ', next_line):
