...
class CVParser:
    ...
    def _extract_linkedin(self, text: str) -> Optional[str]:
        matches = re.findall(r'(?:https?://)?(?:www\.)?linkedin\.com/in/[\w-]+', text, re.IGNORECASE)
        return str(matches[0]) if matches else None
    ...
    def _extract_experience(self, text: str) -> List[Dict[str, str]]:
        section = self._extract_section(text, ["experience", "work experience", "employment", "professional experience"])
        if not section:
            return []
        ...
        for line in lines:
            ...
            if year_match:
                if current_entry:
                    experiences.append(current_entry)
                current_entry = {"title": line.split(year_match.group(0))[0].strip(), "period": year_match.group(0), "description": ""}
            elif current_entry:
    ...
    def _extract_summary(self, text: str) -> Optional[str]:
        section = self._extract_section(text, ["summary", "professional summary", "profile", "about", "objective"])
        if section:
