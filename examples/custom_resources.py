...
class CustomResourceManager:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for resource in self.resources:
            ...
            stats["by_platform"][resource.platform] = (
                stats["by_platform"].get(resource.platform, 0) + 1
            )

        return stats

    def recommend_resources(
        self, skill: str, current_level: str, budget: float = None, max_hours: float = None
    ) -> List[LearningResource]:
        """
...
def demo_usage():
    ...
    recommendations = manager.recommend_resources(
        skill="Go", current_level="beginner", budget=50, max_hours=20
    )
    print("Top recommendations for Go (beginner → intermediate):")
    for i, r in enumerate(recommendations, 1):
        print(f"  {i}. {r.title}")
        print(f"     Platform: {r.platform} | Duration: {r.duration_hours}h | Cost: ${r.cost}")
        print(f"     Rating: {r.rating}⭐ | Level: {r.level}")
